...
df = pd.read_csv(url, sheet_name=None)  # Lê todas as abas

# --- Ler aba de login ---
url_login = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet=Login"
df_login = pd.read_csv(url_login)
# Colunas: A = usuario, B = senha
usuarios_validos = dict(zip(df_login.iloc[:,0], df_login.iloc[:,1]))
...
# --- Validação de login ---
if login_btn:
    if usuario_input in usuarios_validos and senha_input == str(usuarios_validos[usuario_input]):
...
# --- Só mostra o dashboard se estiver logado ---
if st.session_state.logado:
    ...
    st.title("🔐 Por favor, faça login para acessar o dashboard.")
